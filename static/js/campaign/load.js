function add_new_campaign(data) {
    ...
    const count = getRecipientCount(data.id);

    campaignCard.innerHTML = generateCampaignCardHTML(data, count); // <<== Here!
...
function generateCampaignCardHTML(data, count) {
  ...
  const recipientsCount = count; // Replace with actual dynamic count
  const actionNeeded = (!data.dates || recipientsCount === 0);
    ...
    <div class=" text-white rounded-xl p-5 w-full max-w-md">
      ...
      ${actionNeeded ? `
        <div class="flex items-center text-red-400 font-medium text-sm mt-3">
          ...
          Action Needed: Add recipients or set a date.
        </div>
      ` : `
...
function getRecipientCount(id) {
  //get the campign id (passed)
  //
}